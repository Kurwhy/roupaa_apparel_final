...
<head>
    ...
    <title>ROUPAA | Lupa Password</title>
...
<body
    ...
    <a href="{{ route('login') }}" class="fixed top-4 left-4 md:top-8 md:left-8 z-50 flex items-center gap-2 group animate-fade-in">
        <div
    ...
    <main
        ...
        <header class="text-center space-y-2 mb-10 animate-slide-up opacity-0" style="animation-delay: 0.2s;">
            <h1 class="text-on-background font-headline text-3xl font-bold tracking-tight">Lupa Password?</h1>
            <p class="text-on-surface-variant font-body text-xs tracking-wider opacity-80">
                Masukkan email terdaftar. Kami akan mengirimkan link reset password ke inbox kamu.
            </p>
        </header>
        ...
        @if (session('success'))
            <div
                class="mb-6 flex items-start gap-3 bg-emerald-900/30 border border-emerald-500/40 text-emerald-300 px-4 py-3 rounded-lg text-xs font-body tracking-wide animate-fade-in">
        ...
        <form method="POST" action="/forgot-password" novalidate class="space-y-6">
            @csrf
            ...
            <div class="group animate-slide-up opacity-0" style="animation-delay: 0.3s;">
                ...
                <p
                    class="mt-2 text-[10px] text-on-surface-variant/50 font-label tracking-wider flex items-center gap-1">
                    <svg class="w-3 h-3 text-primary/60 flex-shrink-0" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20A10 10 0 0012 2z" />
                    </svg>
                    Link reset akan dikirim ke email ini. Pastikan email aktif dan bisa diakses.
                </p>
            </div>
